...
-- definition's first parking spot. Returns (pos, rot) or (nil, nil) on miss.
local function resolveGaragePosRot(garageId)
  if not garageId then return nil end
  local garage = freeroam_facilities and freeroam_facilities.getFacility and freeroam_facilities.getFacility("garage", garageId) or nil
  if garage then
    local parkingSpots = freeroam_facilities.getParkingSpotsForFacility(garage)
    if parkingSpots and parkingSpots[1] then
      return parkingSpots[1].pos, parkingSpots[1].rot or quat(0, 0, 0, 1)
    end
  end
  if bcm_garages and bcm_garages.getAllDefinitions then
    local defs = bcm_garages.getAllDefinitions()
    local def = defs and defs[garageId]
    if def and def.parkingSpots and def.parkingSpots[1] then
      local spot = def.parkingSpots[1]
      local p = vec3(spot.pos[1], spot.pos[2], spot.pos[3] + 1)
      local d = spot.dir or {0, 1, 0}
      local angle = math.atan2(d[1], d[2])
      local r = quat(0, 0, math.sin(angle/2), math.cos(angle/2))
      return p, r
    end
  end
  return nil
...
spawnAtStarterGarage = function()
  ...
  do
    ...
    rawset(_G, 'bcm_pendingStartGarageId', nil)
    if pending then
      pos, rot = resolveGaragePosRot(pending)
      if pos then
        log("I", logTag, "Using pendingStartGarageId '" .. tostring(pending) .. "' as spawn position")
      else
        log("W", logTag, "pendingStartGarageId '" .. tostring(pending) .. "' has no def/parkingSpot, falling through")
      end
    end
  end
  ...
  if not pos and bcm_garages and bcm_garages.getOwnedGaragesOnCurrentMap then
    local owned = bcm_garages.getOwnedGaragesOnCurrentMap()
    if owned and #owned == 1 then
      pos, rot = resolveGaragePosRot(owned[1].id)
      if pos then
        log("I", logTag, "Auto-selected single owned garage '" .. tostring(owned[1].id) .. "'")
      end
    end
  end
  ...
  -- Fallback: try backup garage on current map (e.g. Italy has no starter but has backups)
  if not pos and bcm_garages then
    local allDefs = bcm_garages.getAllDefinitions()
    if allDefs then
      for garageId, def in pairs(allDefs) do
        if def.isBackupGarage and def.parkingSpots and def.parkingSpots[1] then
          local spot = def.parkingSpots[1]
          pos = vec3(spot.pos[1], spot.pos[2], spot.pos[3] + 1)
          local d = spot.dir or {0, 1, 0}
          local angle = math.atan2(d[1], d[2])
          rot = quat(0, 0, math.sin(angle/2), math.cos(angle/2))
          log("I", logTag, "Using backup garage '" .. garageId .. "' as spawn position")
          break
        end
      end
    end
  ...
  -- Fallback: if no garage found, try travel node position
  if not pos and bcm_multimap then
    local currentMap = bcm_multimap.getCurrentMap() or getCurrentLevelIdentifier()
    local nodes = bcm_multimap.getNodesForCurrentMap()
    if nodes then
      for nodeId, node in pairs(nodes) do
        if node.center then
          pos = vec3(node.center[1], node.center[2], node.center[3] + 1)
          local r = node.rotation or {0, 0, 0, 1}
          rot = quat(r[1], r[2], r[3], r[4])
          log("I", logTag, "Using travel node '" .. nodeId .. "' as spawn position on " .. tostring(currentMap))
          break
        end
      end
    end
  ...
  -- Final fallback to hardcoded WC coordinates (should only happen on west_coast_usa)
  if not pos then
    log("W", logTag, "No garage or travel node found, using fallback coordinates")
...
onCareerActive = function(active, isNewSave)
  ...
  if not isNewSave then
    ...
    -- If no vehicle is spawned, place player at last known position in walking mode
    core_jobsystem.create(function(job)
