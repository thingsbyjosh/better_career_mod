...
local function formatSaveSlotForUi(saveSlot)
  ...
  local propertiesData = jsonReadFile(autosavePath .. "/career/bcm/properties.json")
  ...
  if careerData and careerData.level and propertiesData and type(propertiesData.ownedProperties) == "table" then
    ...
    local garageDefsPath = "/levels/" .. levelName .. "/garages_" .. levelName .. ".json"
    local garageDefs = jsonReadFile(garageDefsPath)
    local starterGarages = {}
    if type(garageDefs) == "table" then
      for _, def in ipairs(garageDefs) do
        if type(def) == "table" and def.id and ownedIds[def.id] then
          -- Guard against cross-map leaks: if the def explicitly declares a
          -- mapName that mismatches the active level, skip it. Defs without a
          -- mapName are accepted (the file is the level-specific JSON).
          local onMap = true
          if def.mapName and def.mapName ~= levelName then onMap = false end
          if onMap then
            table.insert(starterGarages, { id = def.id, displayName = def.name or def.id })
          end
        end
      end
    end
