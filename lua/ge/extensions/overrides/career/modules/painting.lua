...
-- BCM: Override callback to use tier-gated paint when bcm_paintApp is available
local function onComputerAddFunctions(menuData, computerFunctions)
  ...
  for _, vehicleData in ipairs(menuData.vehiclesInGarage) do
    ...
    local compId = menuData.computerFacility.id

    local computerFunctionData = {
      id = "painting",
      label = "Painting",
      callback = function()
        -- BCM: redirect to tier-gated paint if available
        if bcm_paintApp and bcm_paintApp.startPaint then
          bcm_paintApp.startPaint(invId, compId)
        else
          start(invId, compId)
        end
      end,
    ...
    local reason = career_modules_permissions.getStatusForTag({"painting", "vehicleModification"}, {inventoryId = invId})
    ...
    computerFunctions.vehicleSpecific[invId][computerFunctionData.id] = computerFunctionData
  end
end
...
-- BCM: clear painting state without reverting vehicle (used by bcm_paintApp after apply)
local function clearPaintingSession()
  paintingActive = nil
