...
-- BCM: Blacklist ALL vanilla delivery JSONs so only bcm_* prefixed files load.
-- bcm_vehicles.deliveryVehicles.json now defines all vehicle + trailer filters.
local bcmBlacklist = {
  ...
  ["/gameplay/delivery/bcm_planex.deliveryParcels.json"] = true,
}
...
M.onCareerActivated = function()
  ...
  -- BCM: fail-open on nil facility — vanilla progress.lua crashes indexing
  -- nil.isTutorialForCargoDelivery during map switches when generator hasn't
  -- reloaded the new map's facilities yet.
  local origIsFacilityVisible = dProgress.isFacilityVisible
  dProgress.isFacilityVisible = function(facId, isCargoDeliveryTutorialActive)
    if not dGenerator.getFacilityById(facId) then return true end
    return origIsFacilityVisible(facId, isCargoDeliveryTutorialActive)
  end

  -- BCM: ensure delivery facilities have accessPointsByName before vanilla's
  -- general.lua iterates them with pairs(). BCM zone-based facilities can have
  -- nil accessPointsByName during map switch or before resolution.
  local origOnGetRawPoiListForLevel = dGeneral.onGetRawPoiListForLevel
  dGeneral.onGetRawPoiListForLevel = function(levelIdentifier, elements)
    local origGetByType = freeroam_facilities.getFacilitiesByType
    freeroam_facilities.getFacilitiesByType = function(facType)
      local all = origGetByType(facType)
      if facType ~= "deliveryProvider" then return all end
      for _, fac in ipairs(all or {}) do
        if not fac.accessPointsByName then
          fac.accessPointsByName = {}
        end
      end
      return all
    end
    local ok, err = pcall(origOnGetRawPoiListForLevel, levelIdentifier, elements)
    freeroam_facilities.getFacilitiesByType = origGetByType
    if not ok then log('E', 'bcm.generator', 'onGetRawPoiListForLevel error: ' .. tostring(err)) end
  end
...
local function getMoneyRewardForParcelItem(item, distance)
  ...
  local distanceExp = 1.3 + math.sqrt(item.slots)/100 -- BCM: steeper distance scaling for long hauls
  local pricePerM = 5 + math.pow(item.weight, 0.9)
  local modMultiplier = 1 -- BCM: enable modifier price impact
...
local function finalizeParcelItemDistanceAndRewards(item)
  ...
  item.rewards = {
    ...
    logistics = baseXP + round(distance/800) , -- BCM: better XP scaling for distance
    ["logistics-delivery"] = baseXP + round(distance/800) + (template.isTutorialParcel and 10 or 0)
  }
...
local function getDeliveryParcelTemplates()
  if not parcelItemTemplates then
    ...
    for _,file in ipairs(files) do
      if bcmBlacklist[file] then goto bcmSkipParcel end -- BCM: skip blacklisted files
      ...
      ::bcmSkipParcel:: -- BCM: goto target for blacklisted files
    end
    ...
    for i = 1, typeAmount do
      ...
      if not template then
        ...
        elseif generator.type == "parcelReceiver" then
          ...
          if not originAp then
            log("E","","Could not find AP in facility " .. originFac.id .. " with providing type " .. template.logisticType)
            goto continue
          end
...
local function getDeliveryVehicleTemplates()
  if not vehicleFilterTemplates then
    ...
    for _,file in ipairs(files) do
      if not bcmBlacklist[file] then -- BCM: skip blacklisted files
...
local function triggerVehicleOfferGenerator(fac, generator, timeOffset)
  ...
  for  i = 1, count do
    ...
    local filterTemplate = vehicleFilterTemplatesById[generator.filter]
    if not filterTemplate then
      log('W', 'bcm.generator', 'Skipping ' .. tostring(generator.type) .. ' in ' .. tostring(fac.id)
        .. ': missing/unknown filter "' .. tostring(generator.filter) .. '" (add filter field to JSON)')
      goto continue
    end
    local vehType = filterTemplate.type
    ...
    local item = {
      ...
      vehicle = {
        ...
        unlockTag = filterTemplate and filterTemplate.unlockTag or nil,
      },
...
-- Vanilla facility materialStorages use IDs like "soil", "sand" etc.
-- BCM materials use "bcmSoil", "bcmSand" etc.
-- This lookup tries both, and returns a safe stub to prevent nil crashes in general.lua POI code.
local materialFallbackStub = {type = "dryBulk", density = 1.0, money = 0, name = "Unknown", icon = "rocks"}
M.getMaterialsTemplatesById = function(id)
  local result = materialTemplatesById[id]
  if not result and id then
    result = materialTemplatesById["bcm" .. id:sub(1,1):upper() .. id:sub(2)]
  end
  return result or materialFallbackStub
end
      ...
      for _, elem in ipairs(fac.manualAccessPoints or {}) do
        ...
        for _, sitesFromFile in ipairs(sites) do
          ...
          if ps then
            ...
            ap.logisticTypesReceivedLookup = (elem.logisticTypesReceived and next(elem.logisticTypesReceived)) and tableValuesAsLookupDict(elem.logisticTypesReceived) or deepcopy(fac.logisticTypesReceivedLookup)
            ap.logisticTypesProvidedLookup = (elem.logisticTypesProvided and next(elem.logisticTypesProvided)) and tableValuesAsLookupDict(elem.logisticTypesProvided) or deepcopy(fac.logisticTypesProvidedLookup)
...
local function setup(loadData)
  -- Force fresh facility load on every level start. Vanilla assumes one-map careers
  -- and never resets this flag; multimap needs it reset so the new map's facilities
  -- are read from freeroam_facilities instead of keeping the previous map's cache.
...
-- BCM: monkey-patch vanilla general.lua to hide "XX parcels available" POI text when debugMode is off
local function patchGeneralPOI()
  local dGeneral = career_modules_delivery_general
  if not dGeneral or not dGeneral.onActivityAcceptGatherData then return end
  local originalGatherData = dGeneral.onActivityAcceptGatherData
  dGeneral.onActivityAcceptGatherData = function(elemData, activityData)
    originalGatherData(elemData, activityData)
    -- If debug mode is off, strip parcel props from POI data
    local debugMode = extensions.bcm_settings and extensions.bcm_settings.getSetting and extensions.bcm_settings.getSetting('debugMode')
    if not debugMode then
      for _, poi in ipairs(activityData) do
        if poi.props then
          local filtered = {}
          for _, prop in ipairs(poi.props) do
            if not (prop.keyLabel and prop.keyLabel:find("parcel")) then
