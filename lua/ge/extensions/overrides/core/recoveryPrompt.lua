...
local conditions = {
  ...
  vehicleSlow = function(type, vehId)
    if not vehId or vehId < 0 then return true end
...
local function addTaxiButtons()
  ...
  -- untouched.
  local allFacilities = freeroam_facilities.getFacilitiesByType("garage") or {}
  local garages = allFacilities
  if bcm_garages and bcm_garages.getOwnedGaragesOnCurrentMap then
    local ownedHere = bcm_garages.getOwnedGaragesOnCurrentMap() or {}
    local ownedSet = {}
    for _, g in ipairs(ownedHere) do ownedSet[g.id] = true end
    garages = {}
    for _, facility in ipairs(allFacilities) do
      if facility and facility.id and ownedSet[facility.id] then
        table.insert(garages, facility)
      end
    end
    log('I', 'recoveryPrompt', string.format('taxi: %d garages on current map after per-map filter (was %d)', #garages, #allFacilities))
  end
  local discoveredGarages = 0
...
local function buttonPressed(buttonId, target)
  if career_career and career_career.isActive() and not gameplay_walk.isWalking() and not (gameplay_missions_missionManager and gameplay_missions_missionManager.getForegroundMissionId()) then
...
local function addButtonsForLevel(level)
  for _, context in ipairs({
    {
      root = "/root/sandbox/career/",
      enabled = function() return career_career and career_career.isActive() and not (gameplay_missions_missionManager and gameplay_missions_missionManager.getForegroundMissionId()) end,
    },
    {
      root = "/root/sandbox/mission/",
      enabled = function() return gameplay_missions_missionManager and gameplay_missions_missionManager.getForegroundMissionId() end,
    }
  }) do
...
local function addBCMTimeEntries()
  core_quickAccess.addEntry({
    ...
    generator = function(entries)
      ...
      -- Get current BCM time data
      local timeData = bcm_timeSystem.getRadialTimeData()
      if not timeData then return end

      -- Time display entry (read-only info, not clickable)
      table.insert(entries, {
        title = timeData.timeString .. "  |  Day " .. timeData.dayNumber,
        icon = "clock",
        priority = 10,
        enabled = false,
        uniqueID = "bcm_time_display",
        ignoreAsRecentActionForCategory = "sandbox"
      })
      ...
      table.insert(entries, {
        ...
        priority = 11,
        enabled = true,
        uniqueID = "bcm_sleep_action",
        ignoreAsRecentActionForCategory = "sandbox",
        onSelect = function()
          if bcm_sleepManager and bcm_sleepManager.requestSleep then
            bcm_sleepManager.requestSleep()
          end
          return {"hide"}
        end
      })
...
local function onBeforeRadialOpened()
  ...
  addBCMTimeEntries()  -- BCM time display + Sleep button
end
