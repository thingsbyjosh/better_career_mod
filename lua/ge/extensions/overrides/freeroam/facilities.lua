...
local function parseFacility(f, facilityType, facilitiesTypeList, levelDir, fileDir, fileName, index)
  ...
  f.type = facilityType
  if not f.preview or f.preview == "" then
    -- Try garage image by garageId, then by facility id
    local candidates = {}
    if f.garageId then table.insert(candidates, fileDir .. f.garageId .. ".png") end
    if f.id then table.insert(candidates, fileDir .. f.id .. ".png") end
    table.insert(candidates, fileDir .. "defaultFacility.jpg")
    table.insert(candidates, levelDir .. "defaultFacility.jpg")
    f.preview = fileExistsDefault(candidates, missingPreview)
  else
...
local function getFacilities(levelName)
  ...
  if not facilitiesByLevel[levelName] then
    ...
    if levelInfo then
      ...
      local foundFiles = FS:findFiles(levelInfo.dir.."/facilities/", '*.facilities.json', -1, false, true)
      for _,file in ipairs(foundFiles or {}) do
...
local function onGetRawPoiListForLevel(levelIdentifier, elements)
  ...
  -- Travel nodes are injected dynamically by bcm_multimap, not from facilities JSON
end
  ...
  for _, elem in ipairs(elemData) do
    if elem.facility then
      ...
      if elem.type == "dealership" then
        data.buttonLabel = "Inspect Property"
        data.buttonFun = function()
          local dealerName = elem.facility.name or "This dealership"
          guihooks.trigger("BCMDealershipComingSoon", {
            facilityId = elem.facility.id,
            name = dealerName,
          })
        end
        data.props = {}
      ...
      if elem.type == "dragstrip" then
        ...
        data.buttonFun = function() gameplay_drag_dragBridge.openHistoryScreen(elem.facility) end
