...
local loadCountryPoolFromJson
local getCurrentPoliceConfigs
-- FMOD queue
...
-- Loads a country-specific curated police pool from gameplay/police/pools/<country>.json.
-- Returns the configs array on success, nil on missing/malformed/empty file (triggers
-- vanilla-police fallback at the caller).
loadCountryPoolFromJson = function(country)
  ...
  local path = "gameplay/police/pools/" .. slug .. ".json"
  ...
  if not ok or type(data) ~= "table" or type(data.configs) ~= "table" then
    log('W', logTag, 'malformed pool file: ' .. path)
    return nil
  end
  if #data.configs == 0 then return nil end -- empty pool = vanilla fallback
  return data.configs
end
...
getCurrentPoliceConfigs = function()
  ...
  if bcm_multimap and bcm_multimap.getCurrentMapInfo then
    local ok, info = pcall(bcm_multimap.getCurrentMapInfo)
    if ok and type(info) == "table" then
      country = info.country
    end
  end
  return loadCountryPoolFromJson(country)
end

validateConfigPool = function()
  local configs = getCurrentPoliceConfigs()
  if not configs then
    usingVanillaPoliceFallback = true
    validatedConfigs = nil
    log('W', logTag, 'no curated pool for current map, falling back to vanilla police')
    return nil
  end
  usingVanillaPoliceFallback = false

  local validated = {}
  for _, configPath in ipairs(configs) do
  ...
  if #validated == 0 then
    -- Every config in the curated pool failed to resolve (e.g. missing mods).
    -- Treat same as "no pool" and fall back to vanilla rather than forcing a USA default.
    usingVanillaPoliceFallback = true
    validatedConfigs = nil
    log('W', logTag, 'Config pool: all curated configs missing, falling back to vanilla police')
    return nil
  end
...
spawnPolicePool = function()
  ...
  if usingVanillaPoliceFallback or not validatedConfigs then
    log('I', logTag, 'spawnPolicePool: vanilla-police fallback active for this map, skipping BCM pool spawn')
    return
  end
...
spawnRadarCars = function()
  ...
  if usingVanillaPoliceFallback then
  ...
  -- Use tier 1-2 patrol configs (first 7 entries from validated pool, not interceptors)
  local sourcePool = validatedConfigs
  if not sourcePool or #sourcePool == 0 then
    sourcePool = getCurrentPoliceConfigs()
  end
  if not sourcePool or #sourcePool == 0 then
    log('W', logTag, 'spawnRadarCars: no pool available, skipping')
    return
  end
  local patrolConfigs = {}
  for i = 1, math.min(7, #sourcePool) do
...
onBeamNGTrigger = function(data)
  ...
  if overSpeed <= RADAR_SPEED_TOLERANCE_KMH then return end
