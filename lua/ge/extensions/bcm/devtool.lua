...
-- Round-trip audit forward declarations (plan 100.5-05)
local structuralEqual
local runRoundTripForMode
local debugExportRoundTrip
...
exportDeliveryFacilitiesJson = function(levelName, facilityList)
  ...
  for _, f in ipairs(facilityList) do
    ...
    local entry = {
      ...
      tags = f.tags or setmetatable({}, {__jsontype = "array"}),
...
exportAllTravelNodes = function()
  ...
  for _, node in ipairs(travelNodes) do
    ...
    -- Preserve the WIP category (road/port/airport/border/restStop) in a
    -- separate field before overwriting `type` with the file discriminator.
    -- Without this the round-trip silently downgrades every node to "road"
    -- because the loader has no way to recover the original category.
    obj.nodeType = node.type or "road"
    obj.type = "travelNode"
    obj.isExisting = nil  -- runtime-only flag, never persist
    table.insert(objects, obj)
  end
...
structuralEqual = function(a, b, path)
  path = path or "root"
  ...
  if type(a) ~= "table" then
    if a ~= b then
      -- Treat NaN as equal to NaN (avoid infinite failure); otherwise strict.
      if type(a) == "number" and a ~= a and b ~= b then return true, nil end
      return false, string.format("%s: value mismatch (%s vs %s)", path, tostring(a), tostring(b))
    end
    return true, nil
  end
  -- Both are tables. Compare as a set of key-value pairs (order-agnostic).
  local seen = {}
  for k, va in pairs(a) do
    seen[k] = true
    local vb = b[k]
    if vb == nil then
      return false, string.format("%s: key '%s' missing in second export", path, tostring(k))
    end
    local ok, err = structuralEqual(va, vb, path .. "." .. tostring(k))
    if not ok then return false, err end
  end
  for k, _ in pairs(b) do
    if not seen[k] then
      return false, string.format("%s: key '%s' present in second export but not first", path, tostring(k))
    end
  end
  return true, nil
end
...
local function buildRoundTripSpecs()
  ...
  local base = getModLevelsPath(levelName)

  local specs = {}

  -- travelNodes: single NDJSON file, fully round-trippable.
  specs.travelNodes = {
    run = exportAllTravelNodes,
    paths = function()
      return { base .. "/facilities/bcm_travelNodes.json" }
    end,
    reload = function()
      local p = base .. "/facilities/bcm_travelNodes.json"
      local objs = readNdjsonFile(p)
      if not objs then return "read failed: " .. p end
      ...
      for _, obj in ipairs(objs) do
        ...
        if t == "mapInfo" then
          newMapInfo = obj
        elseif t == "travelNode" then
          table.insert(newNodes, obj)
        end
      end
      mapInfo = newMapInfo
      travelNodes = newNodes
      return nil
    end,
  }

  -- gasStations: single JSON file, array of station objects. Simple round-trip.
  specs.gasStations = {
    run = exportAllGasStations,
    paths = function()
      return { base .. "/facilities/bcm_gasStations.json" }
    end,
    reload = function()
      local p = base .. "/facilities/bcm_gasStations.json"
      local arr = jsonReadFile(p)
      if type(arr) ~= "table" then return "read failed: " .. p end
      gasStations = arr
      return nil
    end,
  }
  ...
  specs.garages = {
    ...
    paths = function()
      return {
        ...
        base .. "/facilities.sites.json",
        base .. "/garages_" .. levelName .. ".json",
        base .. "/facilities/delivery/bcm_garages.facilities.json",
        -- items.level.json is NDJSON but jsonReadFile can't parse it; we rely
        -- on readNdjsonFile below inside the compare step.
      }
    end,
    ndjsonPaths = function()
      return {
        base .. "/main/MissionGroup/BCM_AREAS/items.level.json",
      }
    end,
    reload = function()
      return "determinism-only: full reload-from-disk not implemented for garages (merge logic with vanilla base)"
    end,
  }

  -- delivery: writes bcm_depots.facilities.json + bcm_depots.sites.json. The
  -- exporter recomputes oldIds from scratch (starting at 9001) on each run
  -- and reshapes parkingSpots from in-memory facility.parkingSpots[] into
  -- flat site entries. A reload-from-disk would need to invert that reshape.
  -- Downgraded to determinism check like garages.
  specs.delivery = {
    run = exportAllDelivery,
    paths = function()
      return {
        base .. "/facilities/delivery/bcm_depots.facilities.json",
        base .. "/facilities/delivery/bcm_depots.sites.json",
      }
    end,
    reload = function()
      return "determinism-only: full reload-from-disk not implemented for delivery (flat site reshape + oldId regeneration)"
    end,
  }
...
runRoundTripForMode = function(mode)
  local specs, err = buildRoundTripSpecs()
  if not specs then return false, { "[setup] " .. tostring(err) } end
  ...
  -- Step 1: first export
  local ok1, err1 = pcall(spec.run)
  if not ok1 then
    table.insert(diffs, "[first export] pcall error: " .. tostring(err1))
    return false, diffs
  end

  -- Step 2: snapshot disk state
  local jsonPaths = spec.paths()
  local ndjsonPaths = spec.ndjsonPaths and spec.ndjsonPaths() or {}
  local before = {}
  for _, p in ipairs(jsonPaths) do before[p] = jsonReadFile(p) end
  for _, p in ipairs(ndjsonPaths) do before[p] = readNdjsonFile(p) end

  -- Step 3: reload (or note determinism-only)
  local reloadNote = spec.reload()
  if reloadNote then
    table.insert(diffs, "[info] " .. reloadNote)
  end

  -- Step 4: second export
  local ok2, err2 = pcall(spec.run)
  if not ok2 then
    table.insert(diffs, "[second export] pcall error: " .. tostring(err2))
    return false, diffs
  end

  -- Step 5: snapshot disk again
  local after = {}
  for _, p in ipairs(jsonPaths) do after[p] = jsonReadFile(p) end
  for _, p in ipairs(ndjsonPaths) do after[p] = readNdjsonFile(p) end

  -- Step 6: structural compare per file
  local realDiffCount = 0
  local allPaths = {}
  for _, p in ipairs(jsonPaths) do table.insert(allPaths, p) end
  for _, p in ipairs(ndjsonPaths) do table.insert(allPaths, p) end

  for _, p in ipairs(allPaths) do
    local a, b = before[p], after[p]
    if a == nil and b == nil then
      -- Nothing was written to this path by either run. Not a failure (the
      -- mode may not touch every file every run, e.g. empty garage list).
    elseif a == nil or b == nil then
      table.insert(diffs, string.format("[%s] one export produced a file and the other did not (before=%s after=%s)",
        p, tostring(a ~= nil), tostring(b ~= nil)))
      realDiffCount = realDiffCount + 1
    else
      local okEq, errEq = structuralEqual(a, b, p)
      if not okEq then
        table.insert(diffs, errEq)
        realDiffCount = realDiffCount + 1
      end
    end
  end

  return realDiffCount == 0, diffs
end

debugExportRoundTrip = function(mode)
  local modes
  if mode and mode ~= "" and mode ~= "all" then
    modes = { tostring(mode) }
  else
    modes = { "travelNodes", "gasStations", "garages", "delivery" }
  end

  local allPassed = true
  local summary = {}

  log('I', logTag, '=== debugExportRoundTrip: starting audit for ' .. #modes .. ' mode(s) ===')
  print('[roundtrip] === starting audit for ' .. #modes .. ' mode(s) ===')

  for _, m in ipairs(modes) do
    local passed, diffs = runRoundTripForMode(m)
    -- Split info notes from real diffs for cleaner reporting.
    local realDiffs = {}
    local infoNotes = {}
    for _, d in ipairs(diffs or {}) do
      if d:sub(1, 6) == "[info]" then
        table.insert(infoNotes, d)
      else
        table.insert(realDiffs, d)
      end
    end

    if passed then
      local tag = #infoNotes > 0 and "PASS (determinism-only)" or "PASS"
      log('I', logTag, string.format('[roundtrip] %s: %s', m, tag))
      print(string.format('[roundtrip] %s: %s', m, tag))
      for _, n in ipairs(infoNotes) do
        log('I', logTag, '  ' .. n)
        print('  ' .. n)
      end
      table.insert(summary, { mode = m, passed = true, diffs = realDiffs, info = infoNotes })
    else
      allPassed = false
      log('E', logTag, string.format('[roundtrip] %s: FAIL (%d diffs)', m, #realDiffs))
      print(string.format('[roundtrip] %s: FAIL (%d diffs)', m, #realDiffs))
      for _, n in ipairs(infoNotes) do
        log('I', logTag, '  ' .. n)
        print('  ' .. n)
      end
      for _, d in ipairs(realDiffs) do
        log('E', logTag, '  ' .. d)
        print('  ' .. d)
      end
      table.insert(summary, { mode = m, passed = false, diffs = realDiffs, info = infoNotes })
    end
  end

  log('I', logTag, '=== debugExportRoundTrip: done, overall ' .. (allPassed and 'PASS' or 'FAIL') .. ' ===')
  print('[roundtrip] === overall ' .. (allPassed and 'PASS' or 'FAIL') .. ' ===')
  return allPassed, summary
end

M.debugExportRoundTrip = debugExportRoundTrip
