...
local BUYER_PREMIUM_CAPABLE = { enthusiast = true, desperate = true, clueless = true }

-- Re-roll weights when non-premium archetype drawn in overpriced zone
local BUYER_OVERPRICED_REROLL = { enthusiast = 40, desperate = 30, clueless = 30 }
local BUYER_OVERPRICED_ORDER = { "enthusiast", "desperate", "clueless" }
...
-- ============================================================================
-- Price rounding — human-natural counter-offer amounts
-- ============================================================================
...
roundToNicePrice = function(cents)
  ...
  if dollars >= 50000 then
    step = 100000   -- $1,000 steps for $50k+
  elseif dollars >= 10000 then
    step = 50000    -- $500 steps for $10k-$50k
  elseif dollars >= 2000 then
    step = 10000    -- $100 steps for $2k-$10k
  else
    step = 5000     -- $50 steps for under $2k
  end
...
computeThreshold = function(listingSeed, archetypeKey, askingPriceCents)
  ...
  -- firmness 0 -> modePercent 0.25 (very flexible)
  -- firmness 9 -> modePercent 0.03 (very firm)
  local modePercent = 0.25 - (firmness / 9) * 0.22

  -- Sigma adds randomness to acceptance range
  local sigma = 0.05 + pricingEngine.lcgFloat(seed) * 0.08

  local modeCents = math.floor(askingPriceCents * (1 - modePercent))
  local sigmaCents = math.floor(askingPriceCents * sigma)
...
transitionMood = function(currentMood, offerCents, session, archetypeKey)
  ...
  local baselinePriceCents = session.baselinePriceCents or 1
  local effectiveFloor = getEffectiveFloor(session)

  -- Convergence: how close is the offer to the floor (0 = far, 1 = at floor)
  local gap = baselinePriceCents - effectiveFloor
  local convergence = 0
  if gap > 0 then
    convergence = math.max(0, math.min(1, (offerCents - effectiveFloor) / gap))
  end

  -- Offer ratio drives base degradation
  local offerRatio = offerCents / baselinePriceCents
  local baseDegrade = 0
  if offerRatio < 0.70 then
    baseDegrade = 2
  elseif offerRatio < 0.85 then
    baseDegrade = 1
  end

  -- Apply archetype drain rate multiplier
  local drainProfile = ARCHETYPE_DRAIN_PROFILE[archetypeKey] or { drainRate = 1.0 }
  baseDegrade = math.ceil(baseDegrade * drainProfile.drainRate)

  -- Convergence reduction: closer to floor = less drain
  local degradeReduction = math.floor(baseDegrade * convergence * 0.6)
  local finalDegrade = math.max(0, baseDegrade - degradeReduction)

  -- Brittle archetype: if mood has been good for 3+ rounds then drops, extra +1
  if drainProfile.brittle and session.roundsAtGoodMood and session.roundsAtGoodMood >= 3 and finalDegrade > 0 then
    finalDegrade = finalDegrade + 1
  end

  local newIdx = math.min(#MOOD_LEVELS, currentIdx + finalDegrade)
...
pickMessage = function(session, messageType, currentGameDay)
  ...
  -- Deterministic index from listing context
  local listingId = session.listingId or ""
  local hashSeed = #listingId * 100 + (currentGameDay or 0) + #messageType
  local idx = pricingEngine.lcg(hashSeed) % #pool + 1

  local msg = pool[idx]

  -- Placeholder substitution
  if session.vehicleName then
    msg = msg:gsub("{vehicle}", session.vehicleName)
  end
  ...
  if ok and listingGen and listingGen.applyTypos then
    local typoSeed = pricingEngine.lcg((session.listingSeed or 0) + (currentGameDay or 0) * 77)
...
generateBuyerLatentAttributes = function(archetypeKey, listingData, seed)
  ...
  local isPremiumBuyer = fitScore > 0.8 or urgency > 0.8 or marketKnowledge < 0.3
  ...
  local buyerMax
  if isPremiumBuyer then
    buyerMax = math.floor(marketPrice * (1.0 + maxPremium))
  else
    buyerMax = marketPrice -- non-premium never exceeds market
  end
...
computeBuyerInterestDecay = function(session, event, playerCounterCents)
  ...
  elseif event == "aggressive_counter" then
    -- Player counter well above buyer's max: -0.10 to -0.20
    return -0.10 - lcgF(seed * 66666 + round) * 0.10

  elseif event == "reasonable_counter" then
    -- Player is being reasonable: small recovery
    return 0.05
...
computeBuyerInitialOffer = function(session, seed)
  ...
  return roundToNicePrice(offerCents)
end
...
computeBuyerCounterOffer = function(session, playerCounterCents, seed)
  ...
  -- Effective max shrinks with low interest
  local effectiveMax = math.floor(buyerMax * (0.85 + interest * 0.15))

  local newOffer

  if strategy == "CLOSE_FAST" then
    -- Fast concession: 50% of gap
    local gap = effectiveMax - currentOffer
    newOffer = currentOffer + math.floor(gap * 0.50)

  elseif strategy == "PROBE_LOW" then
    -- Slow concession: 15% of gap
    local gap = effectiveMax - currentOffer
    newOffer = currentOffer + math.floor(gap * 0.15)

  elseif strategy == "SPLIT_DIFF" then
    -- Split difference between current and player's counter
    newOffer = math.floor((currentOffer + playerCounterCents) / 2)
    -- But don't exceed effectiveMax
    newOffer = math.min(newOffer, effectiveMax)

  elseif strategy == "MARKET_ONLY" then
    -- Small increments: 20% of gap
    local gap = effectiveMax - currentOffer
    newOffer = currentOffer + math.floor(gap * 0.20)

  elseif strategy == "NIBBLE" then
    -- Tiny increments: 10% of gap
    local gap = effectiveMax - currentOffer
    newOffer = currentOffer + math.floor(gap * 0.10)

  elseif strategy == "WALK_AWAY" then
    -- Barely moves: 5% of gap
    local gap = effectiveMax - currentOffer
    newOffer = currentOffer + math.floor(gap * 0.05)

  else
    local gap = effectiveMax - currentOffer
    newOffer = currentOffer + math.floor(gap * 0.30)
  end
  ...
  return roundToNicePrice(newOffer)
end
