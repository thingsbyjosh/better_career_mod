...
applyPaint = function(inventoryId, paints, partPaints, totalCostCents)
  ...
  -- Save paints to vehicle config and partConditions
  if vehicleInfo then
    if vehicleInfo.config then
      vehicleInfo.config.paints = deepcopy(paints)
    end

    -- Update partConditions so paint persists across respawns
    if vehicleInfo.partConditions then
      for partPath, partCondition in pairs(vehicleInfo.partConditions) do
        if partCondition.visualState and partCondition.visualState.paint then
          if allPartPaints[partPath] then
            -- This part has a custom paint — preserve it
            local partPaintObj = allPartPaints[partPath]
            partCondition.visualState.paint.odometer = 0
            partCondition.visualState.paint.originalPaints = { deepcopy(partPaintObj), deepcopy(partPaintObj), deepcopy(partPaintObj) }
          elseif partCondition.visualState.paint.originalPaints then
            -- No custom paint — update with global slot paints
            partCondition.visualState.paint.odometer = 0
            partCondition.visualState.paint.originalPaints = deepcopy(paints)
          end
        end
      end
    end
...
closePaint = function(inventoryId, revert)
  ...
  if revert and inventoryId and career_modules_inventory then
