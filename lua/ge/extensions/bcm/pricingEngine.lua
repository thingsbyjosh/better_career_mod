...
local ARCHETYPE_MARKUP_PARAMS = {
  grandmother    = { mean = -0.12, stddev = 0.05  },  -- she underprices
  enthusiast     = { mean =  0.12, stddev = 0.04  },  -- firm, knows value
  flipper        = { mean =  0.0,  stddev = 0.03  },  -- market-aware, tight range
  scammer        = { mean =  0.35, stddev = 0.15  },  -- wide range: broken cars at normal price
  urgent_seller  = { mean = -0.12, stddev = 0.06  },  -- underprices, wider variance
  dealer_pro     = { mean =  0.10, stddev = 0.03  },  -- consistent markup
  clueless       = { mean =  0.0,  stddev = 0.15  },  -- wide range: clueless in both directions
  private_seller = { mean = -0.02, stddev = 0.05  },  -- slightly under market
  curbstoner     = { mean = -0.08, stddev = 0.06  },  -- cheap acquisitions, undercuts market
}
...
applyPsychologicalRounding = function(priceCents, archetypeKey)
  ...
  elseif archetypeKey == "private_seller" then
    ...
    return math.floor(dollars / 500 + 0.5) * 500 * 100

  else
    -- Default: round to nearest $50
    return math.floor(dollars / 50 + 0.5) * 50 * 100
  end
end
