...
processPlayerInit = function(listingId)
  ...
  playerText = playerText:gsub("{vehicle}", session.vehicleName or (language == "es" and "el vehiculo" or "the vehicle"))
...
processOffer = function(listingId, offerCents, metadata)
  ...
  -- Append player message with offerCents and metadata
  local playerPool = language == "es" and PLAYER_MESSAGES_ES or PLAYER_MESSAGES_EN
  ...
  -- v2: Re-bid detection — if player bids lower than previous, apply severe mood drain
  if session.lastPlayerOfferCents and offerCents < session.lastPlayerOfferCents then
    -- Artificially reduce offerCents by 20% for the drain calculation only (makes it look much worse)
    local penaltyOffer = math.floor(offerCents * 0.80)
    session.mood = negotiationEngine.transitionMood(session.mood, penaltyOffer, session, session.archetype)
    log('W', logTag, 'Re-bid lower detected: ' .. offerCents .. ' < previous ' .. session.lastPlayerOfferCents .. ' — severe drain applied')

    -- Check if re-bid caused block
  ...
  -- Store last player offer
  session.lastPlayerOfferCents = offerCents

  -- Increment round
  session.roundCount = (session.roundCount or 0) + 1

  -- v2: Over-asking check — instant acceptance regardless of round count
  ...
  if severity == "insulting" then
    if session.archetype == "scammer" then
      -- Scammer ghosts instead of blocking
  ...
  -- v2: Risky severity — extra mood drain + fall through to counter
  local previousMood = session.mood
  if severity == "risky" then
    session.mood = negotiationEngine.transitionMood(session.mood, offerCents, session, session.archetype)

    -- Check if risky caused block
    if session.mood == "blocked" then
      session.isBlocked = true
      local blockText = negotiationEngine.pickMessage(session, "block_message", gameDay)
      blockText = formatAndSubstitute(session, blockText, nil)
  ...
  -- v3: Evaluate offer with zone-based acceptance + probe turns
  local canAccept = session.roundCount > MIN_EXCHANGES_BEFORE_ACCEPT
  local result = negotiationEngine.evaluateOffer(session, offerCents)

  -- v3: Probe mode — good offer but seller wants to "rascar"
  if result.isProbe then
    session.probeTurnsRemaining = math.max(0, (session.probeTurnsRemaining or 0) - 1)

    -- Pick probe_counter message (positive tone, "almost there")
    local probeCounterCents = negotiationEngine.computeCounterOffer(session, offerCents, gameDay, true)
    local probeText = negotiationEngine.pickMessage(session, "probe_counter", gameDay)
    probeText = formatAndSubstitute(session, probeText, probeCounterCents)

    session._lastCounterCents = probeCounterCents
    scheduleSellerResponse(session, probeText, gameDay)
    -- lastSellerCounterCents is set when the message is actually delivered
    -- (either via onUpdate timer or onBCMNewGameDay for thinking responses)
    fireSessionUpdate(session)
    return session
  end

  if canAccept and result.accepted then
    -- Deal!
    session.dealReached = true
    session.dealPriceCents = offerCents
    session.dealExpiresGameDay = gameDay + DEAL_EXPIRY_DAYS
  ...
  -- Not accepted: transition mood (if not already transitioned for risky)
  if severity ~= "risky" then
    previousMood = session.mood
    session.mood = negotiationEngine.transitionMood(session.mood, offerCents, session, session.archetype)
  end

  -- v2: Track roundsAtGoodMood for brittle archetype
  ...
  -- v2: Threshold mood messages — fire when mood just crossed to a new threshold
  ...
  elseif session.mood == "angry" and previousMood == "angry" then
    -- Was already angry — pre-block warning
    local preBlockText = negotiationEngine.pickMessage(session, "threshold_message_pre_block", gameDay)
    if preBlockText and preBlockText ~= "" then
      thresholdPrefix = formatAndSubstitute(session, preBlockText, nil)
    end
  end

  -- Check if mood transition caused block
  if session.mood == "blocked" then
    session.isBlocked = true
    local blockText = negotiationEngine.pickMessage(session, "block_message", gameDay)
    blockText = formatAndSubstitute(session, blockText, nil)
  ...
  -- Handle scammer pressure flow (legacy)
  ...
  -- Check if scammer already pressured — ghost
  ...
  local counterCents = negotiationEngine.computeCounterOffer(session, offerCents, gameDay)

  -- Safety: if counter is at or below player's offer, seller accepts (absurd to ask for less)
  if counterCents <= offerCents then
    log('I', logTag, 'Counter ' .. counterCents .. ' <= offer ' .. offerCents .. ' — auto-accepting')
    session.dealReached = true
    session.dealPriceCents = offerCents
    session.dealExpiresGameDay = gameDay + DEAL_EXPIRY_DAYS
    local confirmText = negotiationEngine.pickMessage(session, "deal_confirm", gameDay)
  ...
  -- Final offer detection: seller won't move further
  -- isFinalOffer is set by the timer on delivery (not here) so Vue doesn't
  -- hide the offer bar before the message appears and the price is correct.
  if session._isFinalOffer then
    session._isFinalOffer = nil

    local finalText = negotiationEngine.pickMessage(session, "final_offer", gameDay)
    finalText = formatAndSubstitute(session, finalText, counterCents)

    session._lastCounterCents = counterCents
    session._pendingFinalOffer = true  -- timer will set isFinalOffer on delivery
  ...
  -- Prepend threshold message if any
...
processPlayerCounterToNPCBuyer = function(listingId, buyerId, counterCents)
  ...
  if session.buyerInterest then
    ...
    local counterRatio = (buyerMax > 0) and (counterCents / buyerMax) or 1.0
    local event = (counterRatio > 1.10) and "aggressive_counter" or "reasonable_counter"
  ...
  if negotiationEngine.shouldBuyerAcceptCounter(session, counterCents, session.seed + session.roundCount * 777) then
    ...
    local newOffer = negotiationEngine.computeBuyerCounterOffer(session, counterCents, session.seed + session.roundCount * 1234)
    session.lastOfferCents = newOffer

    -- Check if buyer is stuck (can't go higher) — signal final offer
    if newOffer <= prevOffer then
      -- Buyer has reached their max — mark as final offer, show accept/reject only
      session.isBuyerFinalOffer = true
      session.lastOfferCents = prevOffer -- keep previous offer as the final one
